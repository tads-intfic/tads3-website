...
`DefineSystemAction(RecordOff)      `*`// original source text`*  
`class `**`RecordOffAction`**` :   `[`SystemAction`](../object/SystemAction.html)`      `*`// after macro expansion`*
...
` `[`execSystemAction`](#execSystemAction)`  `[`turnOffRecording`](#turnOffRecording)`  `
