...
example, you could set this to point to a battery, or to a vial of oil.
By default, for simplicity, the fuel supply and light source are the
same object.
...
own fuel source, the fuel source object must provide these methods
instead of us.

Our fuel level is the number of turns that we can continue to burn. Each
turn we're lit, we'll reduce the fuel level by one. We'll automatically
extinguish ourself when the fuel level reaches zero.
...
`makeLit (lit)`<span class="rem">OVERRIDDEN</span>
