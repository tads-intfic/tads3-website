<span class="title">TellVagueAction</span><span class="type">class</span>
...
`DefineTopicTAction(TellVague, IndirectObject)      `*`// original source text`*  
`class `**`TellVagueAction`**` :   `[`TopicTAction`](../object/TopicTAction.html)`      `*`// after macro expansion`*
