...
layout: docs
---
<span class="title">TravelDirAction</span><span class="type">class</span>
...
where 'direction' is the direction object (northDirection, etc) for the
desired direction of travel. Note that if you want to use the resulting
object in replaceAction() or one of the similar macros, you'll need to
go directly to the underlying function rather than using the standard
macro, since the macros expect a literal action name rather than an
object. For example:
...
[`actionOfKind`](../object/TravelAction.html#actionOfKind) [`execAction`](../object/TravelAction.html#execAction) [`getConnector`](../object/TravelAction.html#getConnector)
...
`getDirection ( )`<span class="rem">OVERRIDDEN</span>
