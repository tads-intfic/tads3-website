...
layout: docs
---
<span class="title">LieOnAction</span><span class="type">class</span>
...
if the actor is already lying down, just say so; otherwise, ask what
they want to lie on

`DefineTAction(LieOn)      `*`// original source text`*  
`class `**`LieOnAction`**` :   `[`TAction`](../object/TAction.html)*`// after macro expansion`*
