...
NORTH". This isn't a real action that the player can type; it's just an
internal construct that we use to represent the partially resolved
action, when we know that we're addressing another actor but we're still
working on figuring out what we're saying.

`class `**`CommandActorAction`**` :   `[`Action`](../object/Action.html)
