...
layout: docs
---
<span class="title">VersionAction</span><span class="type">class</span>
...
Show version information for the game and the library modules the game
is using.

`DefineSystemAction(Version)      `*`// original source text`*  
`class `**`VersionAction`**` :   `[`SystemAction`](../object/SystemAction.html)*`// after macro expansion`*
