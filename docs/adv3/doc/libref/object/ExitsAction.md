...
layout: docs
---
<span class="title">ExitsAction</span><span class="type">class</span>
...
The "exits" verb. This verb explicitly shows all of the exits from the
current location.

`DefineIAction(Exits)      `*`// original source text`*  
`class `**`ExitsAction`**` :   `[`IAction`](../object/IAction.html)*`// after macro expansion`*
