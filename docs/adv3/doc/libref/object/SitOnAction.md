...
layout: docs
---
<span class="title">SitOnAction</span><span class="type">class</span>
...
if the actor is already sitting, just say so; otherwise, ask what they
want to sit on

`DefineTAction(SitOn)      `*`// original source text`*  
`class `**`SitOnAction`**` :   `[`TAction`](../object/TAction.html)`      `*`// after macro expansion`*
