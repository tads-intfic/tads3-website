---
---
<span class="title">NonPortable</span><span class="type">class</span>
...
always use one of the concrete subclasses, such as Fixture or Immovable.
This base class doesn't provide the full behavior necessary to make an
object immovable; it's just here as a programming abstraction for the
common elements of all immovable objects.

This class has two purposes. First, it defines some behavior common to
all non-portable objects. Second, you can test an object to see if it's
based on this class to determine whether it's a portable or unportable
type of Thing.
...
feature of its location, so it should be described as such: either
...
`weight`<span class="rem">OVERRIDDEN</span>
...
object, or otherwise contained in another object, its weight and/or bulk
can affect the behavior of the parent object. So, it's simplest to use a
...
item, so we merely need to check if we're fixed in place within the
...
within an actor, such as when creating component parts of an actor
