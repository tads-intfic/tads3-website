...
not important. We use the catch-all action routine to respond to any
command on this object with a flat "that's not important" message, so
that the player can plainly see that there's no point wasting any time
trying to manipulate this object.

We use the "default" catch-all verb verify handling to report our
"that's not important" message, so a decoration can be made responsive
to specific verbs simply by defining an action handler for those verbs.
...
so that they hide the Default verify handlers. In addition, give
decorations a reduced logical rank, so that any in-scope non-decoration
...
`dobjFor(Read)`<span class="rem">OVERRIDDEN</span>
