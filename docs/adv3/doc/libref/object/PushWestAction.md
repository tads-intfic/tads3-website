...
`DefineAction(PushWest, PushTravelDirAction)      `*`// original source text`*  
`class `**`PushWestAction`**` :   `[`PushTravelDirAction`](../object/PushTravelDirAction.html)`      `*`// after macro expansion`*
