<span class="title">SouthwestAction</span><span class="type">class</span>
...
`DefineAction(Southwest, TravelAction)      `*`// original source text`*  
`class `**`SouthwestAction`**` :   `[`TravelAction`](../object/TravelAction.html)`      `*`// after macro expansion`*
