...
`DefineAction(PushEast, PushTravelDirAction)      `*`// original source text`*  
`class `**`PushEastAction`**` :   `[`PushTravelDirAction`](../object/PushTravelDirAction.html)`      `*`// after macro expansion`*
