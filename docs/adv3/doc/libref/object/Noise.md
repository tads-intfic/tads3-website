...
generating the noise. This will ensure that the noise is automatically
in scope whenever the object is in scope (or, more precisely, whenever
the object's contents are in scope) and with the same sense attributes.

By default, when a noise is specifically examined via "listen to", and
the container is visible, we'll mention that the noise is coming from
the container.
...
`soundPresence`<span class="rem">OVERRIDDEN</span>
...
`cannotSeeSource (obs)`<span class="rem">OVERRIDDEN</span>
...
`soundHereDesc ( )`<span class="rem">OVERRIDDEN</span>
