...
layout: docs
---
<span class="title">equivalentStateLister</span><span class="type">object</span>
...
Equivalent list state lister. This shows a list of state names for a set
of otherwise indistinguishable items. We show the state names in
parentheses, separated by commas only (i.e., no "and" separating the
last two items); we use this less verbose format so that we blend into
the larger enclosing list more naturally.

The items to be listed are EquivalentStateInfo objects.

**`equivalentStateLister`**` :   `[`Lister`](../object/Lister.html)
