...
can be touched or otherwise physically manipulated. Fire, smoke, and fog
are examples of this kind of object.
...
`sightPresence`<span class="rem">OVERRIDDEN</span>
...
explicitly override the corresponding verifiers, so that we bypass the
dobjFor(Default) verifier in Intangible. We don't need to do anything
...
`dobjFor(Search)`<span class="rem">OVERRIDDEN</span>
...
`lookInDesc ( )`<span class="rem">OVERRIDDEN</span>
