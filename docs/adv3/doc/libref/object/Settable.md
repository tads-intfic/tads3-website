...
other markers) on a dial, or stops on a sliding switch.
...
expressing the same value. For example, if the setting is numeric, this
...
`dobjFor(Set)`<span class="rem">OVERRIDDEN</span>
...
`dobjFor(SetTo)`<span class="rem">OVERRIDDEN</span>
