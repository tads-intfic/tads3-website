<span class="title">AskAboutAction</span><span class="type">class</span>
...
`DefineConvTopicTAction(AskAbout, IndirectObject)      `*`// original source text`*  
`class `**`AskAboutAction`**` :   `[`ConvTopicTAction`](../object/ConvTopicTAction.html)`      `*`// after macro expansion`*
