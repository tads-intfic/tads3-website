<span class="title">FootnotesStatusAction</span><span class="type">class</span>
...
`DefineSystemAction(FootnotesStatus)      `*`// original source text`*  
`class `**`FootnotesStatusAction`**` :   `[`SystemAction`](../object/SystemAction.html)`      `*`// after macro expansion`*
