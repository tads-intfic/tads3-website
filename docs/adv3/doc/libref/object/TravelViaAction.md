...
layout: docs
---
<span class="title">TravelViaAction</span><span class="type">class</span>
...
action is to allow real actions that cause travel via a connector to be
implemented by mapping to this internal action, which we implement on
the base travel connector class.
...
asking for a direct object almost always expect a simulation object,
returning a non-simulation object here can be problematic. To avoid
this, we return an empty object list by default - this ensures that no
