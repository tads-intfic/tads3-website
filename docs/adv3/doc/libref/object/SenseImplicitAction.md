...
layout: docs
---
<span class="title">SenseImplicitAction</span><span class="type">class</span>
...
object. We'll scan for things that have a presence in the corresponding
sense and describe each one.

`DefineIAction(SenseImplicit)      `*`// original source text`*  
`class `**`SenseImplicitAction`**` :   `[`IAction`](../object/IAction.html)*`// after macro expansion`*
