...
layout: docs
---
<span class="title">Fixture</span><span class="type">class</span>
...
meant for permanent features of rooms that obviously cannot be moved to
a new container, such as walls, floors, doors, built-in bookcases, light
...
normally wouldn't even try to take it or move it. For objects that might
appear portable but turn out to be immovable, other classes are more
appropriate: use Heavy for objects that are immovable simply because
they're very heavy, for example, or Immovable for objects that are
immovable for some non-obvious reason.
