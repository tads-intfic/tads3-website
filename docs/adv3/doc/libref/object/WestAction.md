<span class="title">WestAction</span><span class="type">class</span>
...
`DefineAction(West, TravelAction)      `*`// original source text`*  
`class `**`WestAction`**` :   `[`TravelAction`](../object/TravelAction.html)`      `*`// after macro expansion`*
