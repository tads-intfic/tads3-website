...
layout: docs
---
<span class="title">AttackAction</span><span class="type">class</span>
...
add to the scope all of the actor's followable objects - these are the
objects which the actor has witnessed leaving the actor's present
location

`DefineTAction(Attack)      `*`// original source text`*  
`class `**`AttackAction`**` :   `[`TAction`](../object/TAction.html)`      `*`// after macro expansion`*
