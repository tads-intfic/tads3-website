...
You can leave out the `\<\<else\>\>`, in which
...
You can nest `\<\<if\>\>`'s to any depth -
...
`\<\<one of\>\>` structures can be nested, and
`\<\<if\>\>` and `\<\<one
...
If you look carefully at the `\<\<one of\>\>`
...
You'll notice that the `getNext()` method
