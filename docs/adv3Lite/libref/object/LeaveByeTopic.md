[LeaveByeTopic]{.title}[class]{.type}
...
`class `**`LeaveByeTopic`**` :   `[`GreetingTopic`](../object/GreetingTopic.html)
:::
...
::: desc
Give LeaveByeTopic an even higher matchScore so that it takes precedence
