...
`// listOrder`[OVERRIDDEN]{.rem}[Interface description only]{.rem}
...
`dobjFor(Examine)`[OVERRIDDEN]{.rem}
...
`dobjFor(GetOutOf)`[OVERRIDDEN]{.rem}
...
Connector (normally defined by the asExit() macro. If none of the
room\'s direction properties clearly leads to dest via a TravelConnector
...
`iobjFor(PushTravelGetOutOf)`[OVERRIDDEN]{.rem}
...
`outermostVisibleParent ( )`[OVERRIDDEN]{.rem}
...
`showConnectedMiscContents (pov)`[OVERRIDDEN]{.rem}
...
`showFirstConnectedSpecials (pov)`[OVERRIDDEN]{.rem}
...
`showSecondConnectedSpecials (pov)`[OVERRIDDEN]{.rem}
...
`showStatuslineExits ( )`[OVERRIDDEN]{.rem}
...
`statusName (actor)`[OVERRIDDEN]{.rem}
...
`unmentionRemoteContents ( )`[OVERRIDDEN]{.rem}
