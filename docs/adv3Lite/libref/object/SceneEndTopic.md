[SceneEndTopic]{.title}[class]{.type}
...
::: fdesc
A SceneEndTopic is a topic entry that is triggered when any of the
scenes in its matchObj property ends. \[SCENETOPIC EXTENSION\]

`class `**`SceneEndTopic`**` :   `[`SceneTopic`](../object/SceneTopic.html)
:::
