...
string. If \'idx\' and \'len\' are specified, the give the range of
bytes to include in the hash; the default is to hash the whole array.
:::
...
representations to use for the argument values. The remaining arguments
after \'format\' are the data values to pack.
...
starting at the given index. An \"index out of range\" exception is
thrown if there aren\'t enough bytes in the array to satisfy the
request.
:::
...
UTF-8 representation of the string. If \'idx\' and \'len\' are
...
\'format\' is the format string. Returns a list of the unpacked values.
...
outside of the bounds of the format to be written, the written value is
truncated.
...
exception is thrown if the byte array doesn\'t have enough space to
store the value.
