[DispensingCollective]{.title}[class]{.type}
...
`collectiveAction (np, cmd)`[OVERRIDDEN]{.rem}
...
`dobjFor(TakeFrom)`[OVERRIDDEN]{.rem}
...
`iobjFor(TakeFrom)`[OVERRIDDEN]{.rem}
...
`isCollectiveFor (obj)`[OVERRIDDEN]{.rem}
