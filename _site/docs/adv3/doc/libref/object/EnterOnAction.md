<span class="title">EnterOnAction</span><span class="type">class</span>
...
`DefineLiteralTAction(EnterOn, DirectObject)      `*`// original source text`*  
`class `**`EnterOnAction`**` :   `[`LiteralTAction`](../object/LiteralTAction.html)`      `*`// after macro expansion`*
