<span class="title">LockWithAction</span><span class="type">class</span>
...
`DefineTIAction(LockWith)      `*`// original source text`*  
`class `**`LockWithAction`**` :   `[`TIAction`](../object/TIAction.html)`      `*`// after macro expansion`*
...
`resolveFirst`<span class="rem">OVERRIDDEN</span>

[actions.t](../file/actions.t.html)\[[2515](../source/actions.t.html#2515)\]

<div class="desc">

Resolve the direct object (the lock) first, so that we know what we're
trying to unlock when we're verifying the key. This allows us to
(optionally) boost the likelihood of a known good key for
disambiguation.
