<span class="title">RestartAction</span><span class="type">class</span>
...
`DefineSystemAction(Restart)      `*`// original source text`*  
`class `**`RestartAction`**` :   `[`SystemAction`](../object/SystemAction.html)`      `*`// after macro expansion`*
...
` `[`doRestartGame`](#doRestartGame)`  `[`execSystemAction`](#execSystemAction)`  `
